...
class Navbar extends Component {
  ...
  renderRightMenuItems = () => {
    ...
    const menuItems = LABELS.map((label, index) => {
      return (
        <li key={index}>
          <a className="browse-tablinks" href="/">
            {label}
          </a>
        </li>
      );
  ...
  renderAccountButtons = () => {
    ...
    const loginSelectorPageStyles = this.checkPage("accountSelector")
      ? { color: "white" }
      : {};
    const signUpSelectorPageStyles = this.checkPage("accountSelector")
  ...
  renderNavMenu = side => {
    const { user } = this.props;
    const rightMenuSelectorPageStyles = this.checkPage("accountSelector")
      ? { marginRight: "1em" }
      : {};
    ...
    if (!isLoggedIn(user) && this.checkPage("home") && side === "left") {
      return (
        <ul className="nav navbar-nav">
          <li>
            <a id="home-link" className="browse-tablinks active" href="/">
              HOME
            </a>
          </li>
          <li>
            <a id="second-home-link" className="browse-tablinks" href="/">
              HOME ALT
            </a>
          </li>
          <li>
          ...
          <li>
            <a className="browse-tablinks" href="/blog">
              BLOG
            </a>
          </li>
          <li>
      ...
      !isLoggedIn(user) &&
      this.checkPage("accountSelector") &&
      side === "left"
    ) {
      return (
      ...
      (this.checkPage("home") && side === "center") ||
      (isLoggedIn(user) && side === "left")
    ) {
      return (
        <div
          className={`navbar-header${
            isLoggedIn(user) ? "" : " display-inline-block float-unset"
          }`}
        >
          <a
            className={`navbar-brand${isLoggedIn(user) ? " all-links" : ""}`}
            href="/"
            onClick={event => {
              this.clearSignOn(event);
              openRoute(event, "/");
      ...
      return (
        <ul className="nav navbar-nav">
          ...
          <li>
            <a
              href="/"
              className="main-tablinks"
              onClick={event => {
                stopPropagation(event);
                showOverlay("freelancers-dropdown", event, "main-tablinks");
                dismissOverlay(null, [
                  "account-dropdown",
                  "notification-tray",
                  "message-dropdown",
          ...
          <li>
            <a
              ...
              onClick={event => {
                stopPropagation(event);
                showOverlay("connections-dropdown", event, "main-tablinks");
                dismissOverlay(null, [
                  "account-dropdown",
                  "notification-tray",
                  "message-dropdown",
        ...
        <ul className="nav navbar-nav navbar-right">
          ...
          <li className="after-separator">
            <a
              ...
              onClick={event => {
                stopPropagation(event);
                showOverlay("job-feed-dropdown", event);
                dismissOverlay(null, [
                  "account-dropdown",
                  "notification-tray",
                  "message-dropdown",
          ...
          <li>
            <a
              ...
              onClick={event => {
                stopPropagation(event);
                showOverlay("message-dropdown", event);
                dismissOverlay(null, [
                  "account-dropdown",
                  "notification-tray",
                  "job-feed-dropdown",
          ...
          <li>
            <a
              ...
              onClick={event => {
                stopPropagation(event);
                showOverlay("notification-tray", event);
                dismissOverlay(null, [
                  "account-dropdown",
                  "job-feed-dropdown",
                  "message-dropdown",
          ...
          <li>
            <a
              ...
              onClick={event => {
                stopPropagation(event);
                showOverlay("account-dropdown", event);
                dismissOverlay(null, [
                  "notification-tray",
                  "job-feed-dropdown",
                  "message-dropdown",
